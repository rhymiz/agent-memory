...
Request its `decisions` section when architecture or product direction matters.
...
- **Shared edits:** Read [claims.md](references/claims.md) before acquiring
  ownership. Claim only files needed in the current phase; retain their IDs and
...
- **Discoveries and completion:** Store concise reusable findings or results,
  including evidence and unresolved limits. Skip routine tool activity and
  duplicates. Release all owned claims, including on cancellation when possible.
